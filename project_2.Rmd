...
# determine the number of fatalities per event
damage_per_event <- aggregate(stormdata$TOTAL_DMG, by=list(stormdata$EVTYPE), FUN=sum)
names(damage_per_event) <- c("event","damage")

# determine what is the top 10 of events with the most damage
top10_damage <- head(damage_per_event[order(damage_per_event$damage,decreasing = T),],10)
